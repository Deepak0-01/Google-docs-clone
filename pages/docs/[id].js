...
function Doc() {
    ...
    useEffect(()=>{
        ...
        db.collection("userDocs").doc(session?.user?.email).collection("docs").doc(id).onSnapshot((snapshot)=>{
...
const handleSignOut = ()=>{
    ...
    return (
        ...
        <Icon onClick={()=>router.push("/")} name="article" color="blue" size="5xl"/>
        <div className=" h-10">
          ...
          className="mr-6 rounded-full h-10 w-10"
           ...
           src={session?.user?.image} alt="user_image" />
